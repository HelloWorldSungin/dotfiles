...
const SNAPSHOT_TABLE = "zai_quota_snapshot";
...
async function keyFromClaudeSettings(): Promise<string | undefined> {
  try {
    ...
    if (!baseUrl.includes("z.ai") && !baseUrl.includes("bigmodel.cn")) return undefined;
...
async function resolveApiKey(): Promise<string | undefined> {
  const fromProcess = (process.env.ZAI_API_KEY ?? process.env.GLM_API_KEY)?.trim();
  if (fromProcess) return fromProcess;

  const fromShell =
    (await envFromLoginShell("ZAI_API_KEY")) ?? (await envFromLoginShell("GLM_API_KEY"));
  if (fromShell) return fromShell;

  const baseUrl = process.env.ANTHROPIC_BASE_URL ?? "";
  if (baseUrl.includes("z.ai") || baseUrl.includes("bigmodel.cn")) {
...
function normalizeResponse(body: unknown): QuotaSnapshot | undefined {
  ...
  const limits = Array.isArray(data?.limits) ? data.limits.map(asRecord).filter(Boolean) : [];
  const tokenLimits = limits.filter((limit) => limit?.type === "TOKENS_LIMIT");
  const session = tokenLimits.find((limit) => limit?.unit === 3);
  const weekly = tokenLimits.find((limit) => limit?.unit === 6);
  if (typeof session?.percentage !== "number" || typeof weekly?.percentage !== "number") return undefined;

  return {
    source: "api",
    plan: typeof data?.level === "string" ? data.level : undefined,
    windows: [
      {
        id: "session",
        label: "session · 5 hour",
        percentUsed: clampPercent(session.percentage),
        ...resetFields(session.nextResetTime),
      },
      {
        id: "weekly",
        label: "weekly",
        percentUsed: clampPercent(weekly.percentage),
        ...resetFields(weekly.nextResetTime),
      },
    ],
...
async function getQuota(_input: unknown, context: BabyMenuServerContext): Promise<QuotaResult> {
  const sourceTried = ["quota-api"];
  const apiKey = await resolveApiKey();
  if (!apiKey) return { ok: false, error: "Z.AI API key required", sourceTried };
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      return { ok: false, error: "Z.AI sign-in required", sourceTried };
    }
    ...
    return { ok: false, error: "Z.AI quota unavailable", sourceTried };
  } catch {
    const last = readLastSnapshot(context.db);
    if (last) return { ok: true, data: { ...last, stale: true } };
    return { ok: false, error: "Z.AI quota unavailable", sourceTried };
