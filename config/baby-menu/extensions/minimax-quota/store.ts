...
const EXTENSION_ID = "minimax-quota";
...
export function useMiniMaxQuotaState(): QuotaState {
  return useSyncExternalStore(subscribe, getSnapshot);
}

export async function refreshMiniMaxQuota(): Promise<void> {
  if (inFlight) return;
  if (!window.babyMenu) {
  ...
  try {
    ...
    setState({
      status: "error",
      message: error instanceof Error ? error.message : "Failed to load MiniMax quota",
    });
  } finally {
    inFlight = false;
  }
