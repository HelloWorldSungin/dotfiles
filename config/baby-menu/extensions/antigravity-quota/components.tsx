...
import { refreshAntigravityQuota, useAntigravityState, type AntigravityWindow } from "./store";
...
function CircleProgress({ percentLeft, label }: { percentLeft: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 shrink-0">
      ...
      <span className="text-[9px] uppercase tracking-caps text-ink-label font-mono text-center mt-1 truncate max-w-[72px]">{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="font-mono text-xs text-ink-strong shrink-0">
          {Math.round(percentLeft)}
...
function resetText(value?: string): string | undefined {
...
export function AntigravityQuotaView() {
  const state = useAntigravityState();
  useEffect(() => {
    void refreshAntigravityQuota();
  }, []);

  let primaryWindow: AntigravityWindow | undefined;
  let otherWindows: AntigravityWindow[] = [];

  if (state.status === "ready") {
    // Sort windows so Gemini 3.5 Flash (High) is always first (in the circle)
    const sorted = [...state.snapshot.windows].sort((a, b) => {
      const aLabel = a.label.toLowerCase();
      const bLabel = b.label.toLowerCase();
      if (aLabel.includes("flash (high)")) return -1;
      if (bLabel.includes("flash (high)")) return 1;
      if (aLabel.includes("flash (medium)")) return -1;
      if (bLabel.includes("flash (medium)")) return 1;
      return a.label.localeCompare(b.label);
    });

    primaryWindow = sorted[0];
    otherWindows = sorted.slice(1);
  }
      ...
      <div className="flex items-center justify-between text-xxs uppercase tracking-caps text-ink-label border-b border-line-faint/10 pb-1">
        ...
        {state.status === "ready" ? (
          <span className="flex items-center gap-1.5 text-signal-live">
            ...
            {state.snapshot.stale ? "stale" : "local"}
          </span>
        ) : null}
      </div>
      ...
      {state.status === "ready" ? (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-5 py-1">
            {primaryWindow ? (
              <CircleProgress
                percentLeft={100 - (primaryWindow.percentUsed ?? 0)}
                label={shortenLabel(primaryWindow.label)}
              />
            ) : null}
          ...
          <div className="flex items-center justify-between text-xxs uppercase tracking-caps text-ink-label border-t border-line-faint/10 pt-2 mt-1">
            <span className="font-mono">
              ...
              {primaryWindow?.resetAt ? `resets ${resetText(primaryWindow.resetAt)}` : ""}
            </span>
            <div className="flex items-center gap-1.5 font-mono">
              ...
              <button
                type="button"
                onClick={() => void refreshAntigravityQuota()}
