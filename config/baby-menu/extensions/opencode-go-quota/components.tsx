...
import { refreshGoQuota, useGoQuotaState, type GoWindow } from "./store";
...
function until(resetAt?: string): string | undefined {
...
export function OpenCodeGoQuotaView() {
  const state = useGoQuotaState();
  useEffect(() => {
    void refreshGoQuota();
  }, []);

  let weeklyWindow: GoWindow | undefined;
  let otherWindows: GoWindow[] = [];

  if (state.status === "ready") {
    weeklyWindow = state.snapshot.windows.find((w) => w.id === "weekly") || state.snapshot.windows[0];
    otherWindows = state.snapshot.windows.filter((w) => w.id !== weeklyWindow?.id);
  }
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between text-xxs uppercase tracking-caps text-ink-label border-t border-line-faint/10 pt-2 mt-1">
            <span className="font-mono">
              ...
              {weeklyWindow?.resetAt ? `resets in ${until(weeklyWindow.resetAt)}` : ""}
            </span>
