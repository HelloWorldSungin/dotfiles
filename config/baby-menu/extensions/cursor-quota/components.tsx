...
import { refreshCursorQuota, useCursorQuotaState, type CursorWindow } from "./store";
...
function resetLabel(resetAt?: string): string | undefined {
...
export function CursorQuotaView() {
  const state = useCursorQuotaState();
  useEffect(() => {
    void refreshCursorQuota();
  }, []);

  let primaryWindow: CursorWindow | undefined;
  let otherWindows: CursorWindow[] = [];

  if (state.status === "ready") {
    primaryWindow = state.snapshot.windows.find((w) => w.id === "included_usage") || state.snapshot.windows[0];
    otherWindows = state.snapshot.windows.filter((w) => w.id !== primaryWindow?.id);
  }
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between text-xxs uppercase tracking-caps text-ink-label border-t border-line-faint/10 pt-2 mt-1">
            <span className="font-mono">
              ...
              {primaryWindow?.resetAt ? `resets ${resetLabel(primaryWindow.resetAt)}` : ""}
            </span>
            <div className="flex items-center gap-1.5 font-mono">
              ...
              <button
                type="button"
                onClick={() => void refreshCursorQuota()}
